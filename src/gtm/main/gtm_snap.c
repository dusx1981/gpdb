...
ProcessGetSnapshotCommand(Port *myport, StringInfo message, bool get_gxid)
{
	...
	data = gtm_pq_getmsgbytes(message, sizeof (gxid));
	if (data == NULL)
		ereport(ERROR,
				(EPROTO,
	...
	if ((snapshot = GTM_GetTransactionSnapshot(&txn, 1, &status)) == NULL)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to get a snapshot")));
	...
	gtm_pq_sendbytes(&buf, (char *)&snapshot->sn_xmin, sizeof (GlobalTransactionId));
	gtm_pq_sendbytes(&buf, (char *)&snapshot->sn_xmax, sizeof (GlobalTransactionId));
	gtm_pq_sendbytes(&buf, (char *)&snapshot->sn_recent_global_xmin, sizeof (GlobalTransactionId));
	pq_sendint(&buf, snapshot->sn_xcnt, sizeof (int));
	gtm_pq_sendbytes(&buf, (char *)snapshot->sn_xip,
				 sizeof(GlobalTransactionId) * snapshot->sn_xcnt);
