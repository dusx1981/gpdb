...
main(int argc, char *argv[])
{
	...
	if (GTMDataDir == NULL)
	{
		gtm_write_stderr("GTM data directory must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr("Nodename must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr("Listen_addresses must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
...
ServerLoop(void)
{
	...
	for (;;)
	{
		...
		//Gtm_MemoryContextStats(TopMostMemoryContext);
...
GTM_ThreadMain(void *argp)
{
...

	{
		...
		startup_type = gtm_pq_getbyte(thrinfo->thr_conn->con_port);
		 ...
		 * the type.
		 */
		if (gtm_pq_getmessage(thrinfo->thr_conn->con_port, &inBuf, 0))
			ereport(ERROR,
					(EPROTO,
					 gtm_errmsg("Expecting coordinator ID, but received EOF")));
...

	{
		...
		gtm_pq_endmessage(thrinfo->thr_conn->con_port, &buf);
	...
	if (sigsetjmp(local_sigjmp_buf, 1) != 0)
	{
		...
		if (thrinfo->thr_conn)
			Gtm_EmitErrorReport(thrinfo->thr_conn->con_port);
		else
	...
	for (;;)
	{
		...
		switch(qtype)
		{
			...
			case 'F':
				...
				gtm_pq_getmsgint(&input_message, sizeof (GTM_MessageType));
				gtm_pq_getmsgend(&input_message);
				pq_flush(thrinfo->thr_conn->con_port);
				break;
...
ProcessCommand(Port *myport, StringInfo input_message)
{
	...
	if (myport->remote_type == GTM_NODE_GTM_PROXY)
		gtm_pq_copymsgbytes(input_message, (char *)&proxyhdr, sizeof (GTM_ProxyMsgHeader));
	else
...
ReadCommand(Port *myport, StringInfo inBuf)
{
	 ...
	 * the type.
	 */
	if (gtm_pq_getmessage(myport, inBuf, 0))
				...
				ereport(FATAL,
						(EINVAL,
						 gtm_errmsg("lock file \"%s\" already exists",
								filename),
						  gtm_errhint("Is another GTM (PID %d) running in data directory \"%s\"?",
...
PromoteToActive(void)
{
	...
	if ((fp = fopen(conf_file, PG_BINARY_A)) == NULL)
	{
		ereport(FATAL,
				(EINVAL,
				 gtm_errmsg("could not open GTM configuration file \"%s\": %m",
						conf_file)));
	...
	else
	{
		...
		if (fclose(fp))
			ereport(FATAL,
					(EINVAL,
					 gtm_errmsg("could not close GTM configuration file \"%s\": %m",
							conf_file)));
	}
	return;
}
...
static void ProcessBarrierCommand(Port *myport, GTM_MessageType mtype, StringInfo message)
{
	...
	barrier_id_len = gtm_pq_getmsgint(message, sizeof(int));
	barrier_id = (char *)gtm_pq_getmsgbytes(message, barrier_id_len);
	gtm_pq_getmsgend(message);
