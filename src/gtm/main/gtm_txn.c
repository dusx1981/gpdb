...
GTM_GXIDToHandle_Internal(GlobalTransactionId gxid, bool warn)
{
	...
	else
	{
		if (warn)
			ereport(WARNING,
				(ERANGE, gtm_errmsg("No transaction handle for gxid: %d",
								gxid)));
		return InvalidTransactionHandle;
	}
}
...
GTM_HandleToTransactionInfo(GTM_TransactionHandle handle)
{
	...
	{
		ereport(WARNING,
				(ERANGE, gtm_errmsg("Invalid transaction handle: %d", handle)));
		return NULL;
	}
	...
	{
		ereport(WARNING,
				(ERANGE, gtm_errmsg("Invalid transaction handle (%d), txn_info not in use",
								handle)));
		return NULL;
	}
...
GTM_SetDoVacuum(GTM_TransactionHandle handle)
{
	...
	if (gtm_txninfo == NULL)
		ereport(ERROR, (EINVAL, gtm_errmsg("Invalid transaction handle")));
...
GTM_GetGlobalTransactionIdMulti(GTM_TransactionHandle handle[], int txn_count)
{
	...
	{
		ereport(ERROR, (EINVAL, gtm_errmsg("GTM is running in STANDBY mode -- can not issue new transaction ids")));
	...
	if (GTMTransactions.gt_gtm_state == GTM_SHUTTING_DOWN)
	{
		...
		ereport(ERROR, (EINVAL, gtm_errmsg("GTM shutting down -- can not issue new transaction ids")));
		return InvalidGlobalTransactionId;
	}
	...
	for (ii = 0; ii < txn_count; ii++)
	{
			...
			GlobalTransactionIdIsValid(GTMTransactions.gt_xidVacLimit))
		{
			if (GlobalTransactionIdFollowsOrEquals(xid, GTMTransactions.gt_xidStopLimit))
			{
				...
				ereport(ERROR,
						(ERANGE,
						 gtm_errmsg("database is not accepting commands to avoid wraparound data loss in database ")));
			}
			else if (GlobalTransactionIdFollowsOrEquals(xid, GTMTransactions.gt_xidWarnLimit))
				ereport(WARNING,
				(gtm_errmsg("database must be vacuumed within %u transactions",
					 ...
					 GTM_TransactionHandle txns[])
{
	...
	GTM_TransactionInfo *gtm_txninfo[DUMP_LEN];
	...
	for (kk = 0; kk < txn_count; kk++)
	{
			 ...
			 ii = (ii + 1) % GTM_MAX_GLOBAL_TRANSACTIONS, jj++)
		{
			...
			if (ii == GTMTransactions.gt_lastslot)
			{
				GTM_RWLockRelease(&GTMTransactions.gt_TransArrayLock);
				ereport(ERROR,
						(ERANGE, gtm_errmsg("Max transaction limit reached")));
			}
		}
...
/*
 * Clean up the TransactionInfo slot and gtm_pfree all the palloc'ed memory,
...
clean_GTM_TransactionInfo(GTM_TransactionInfo *gtm_txninfo)
{
	...
	{
		gtm_pfree(gtm_txninfo->gti_gid);
		gtm_txninfo->gti_gid = NULL;
	}
	if (gtm_txninfo->nodestring)
	{
		gtm_pfree(gtm_txninfo->nodestring);
		gtm_txninfo->nodestring = NULL;
	}
}
							  ...
							  int	txn_count)
{
	...
	if (count != txn_count)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to start %d new transactions", txn_count)));
		...
		int status[])
{
	...
	GTM_TransactionInfo *gtm_txninfo[DUMP_LEN];
	GTM_TransactionInfo *remove_txninfo[DUMP_LEN];
	int remove_count = 0;
	int ii;
							 ...
							 char *nodestring)
{
	...
	if (gtm_txninfo->nodestring == NULL)
		gtm_txninfo->nodestring = (char *)Gtm_MemoryContextAlloc(TopMostMemoryContext,
	...
	if (gtm_txninfo->gti_gid == NULL)
		gtm_txninfo->gti_gid = (char *)Gtm_MemoryContextAlloc(TopMostMemoryContext, GTM_MAX_GID_LEN);
...
ProcessBeginTransactionCommand(Port *myport, StringInfo message)
{
	...
	txn_isolation_level = gtm_pq_getmsgint(message, sizeof (GTM_IsolationLevel));
	txn_read_only = gtm_pq_getmsgbyte(message);
	...
	if (txn == InvalidTransactionHandle)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to start a new transaction")));
	...
	if (myport->remote_type == GTM_NODE_GTM_PROXY)
	{
		...
		gtm_pq_sendbytes(&buf, (char *)&proxyhdr, sizeof (GTM_ProxyMsgHeader));
	}
	gtm_pq_sendbytes(&buf, (char *)&txn, sizeof(txn));
	gtm_pq_sendbytes(&buf, (char *)&timestamp, sizeof (GTM_Timestamp));
	gtm_pq_endmessage(myport, &buf);
...
ProcessBkupBeginTransactionCommand(Port *myport, StringInfo message)
{
	...
	txn_isolation_level = gtm_pq_getmsgint(message, sizeof(GTM_IsolationLevel));
	txn_read_only = gtm_pq_getmsgbyte(message);
	client_id = gtm_pq_getmsgint(message, sizeof (uint32));
	memcpy(&timestamp, gtm_pq_getmsgbytes(message, sizeof(GTM_Timestamp)), sizeof(GTM_Timestamp));
...
ProcessBeginTransactionGetGXIDCommand(Port *myport, StringInfo message)
{
	...
	if (gxid == InvalidGlobalTransactionId)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to get a new transaction id")));
...
retry:
	...
	gtm_pq_sendbytes(&buf, (char *)&gxid, sizeof(gxid));
	gtm_pq_sendbytes(&buf, (char *)&timestamp, sizeof (GTM_Timestamp));
	gtm_pq_endmessage(myport, &buf);
...
ProcessBkupBeginTransactionGetGXIDCommand(Port *myport, StringInfo message)
{
	...
	gxid = gtm_pq_getmsgint(message, sizeof(GlobalTransactionId));
	txn_isolation_level = gtm_pq_getmsgint(message, sizeof(GTM_IsolationLevel));
	txn_read_only = gtm_pq_getmsgbyte(message);
	txn_client_id = gtm_pq_getmsgint(message, sizeof (uint32));
	memcpy(&timestamp, gtm_pq_getmsgbytes(message, sizeof(GTM_Timestamp)), sizeof(GTM_Timestamp));
	gtm_pq_getmsgend(message);
...
ProcessBeginTransactionGetGXIDCommandMulti(Port *myport, StringInfo message)
{
	...
	txn_count = gtm_pq_getmsgint(message, sizeof (int));
	...
	for (ii = 0; ii < txn_count; ii++)
	{
		txn_isolation_level[ii] = gtm_pq_getmsgint(message, sizeof (GTM_IsolationLevel));
		txn_read_only[ii] = gtm_pq_getmsgbyte(message);
		txn_connid[ii] = gtm_pq_getmsgint(message, sizeof (GTMProxy_ConnID));
...
retry:
	...
	gtm_pq_sendbytes(&buf, (char *)&start_gxid, sizeof(start_gxid));
	gtm_pq_sendbytes(&buf, (char *)&(timestamp), sizeof (GTM_Timestamp));
	gtm_pq_endmessage(myport, &buf);
...
ProcessBkupBeginTransactionGetGXIDCommandMulti(Port *myport, StringInfo message)
{
	...
	for (ii = 0; ii < txn_count; ii++)
	{
		gxid[ii] = gtm_pq_getmsgint(message, sizeof(GlobalTransactionId));
		txn_isolation_level[ii] = gtm_pq_getmsgint(message, sizeof(GTM_IsolationLevel));
		txn_read_only[ii] = gtm_pq_getmsgbyte(message);
		txn_client_id[ii] = gtm_pq_getmsgint(message, sizeof(uint32));
		txn_connid[ii] = gtm_pq_getmsgint(message, sizeof(GTMProxy_ConnID));
	}
...
ProcessCommitTransactionCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	const char *data = gtm_pq_getmsgbytes(message, sizeof (gxid));
	...
	waited_xid_count = gtm_pq_getmsgint(message, sizeof (int));
	if (waited_xid_count > 0)
	{
		waited_xids = gtm_pq_getmsgbytes(message,
...
ProcessCommitPreparedTransactionCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	GTM_TransactionHandle txn[2];
	GlobalTransactionId gxid[2];
	MemoryContext oldContext;
	int status[2];
	...
	for (ii = 0; ii < txn_count; ii++)
	{
		const char *data = gtm_pq_getmsgbytes(message, sizeof (gxid[ii]));
		if (data == NULL)
			ereport(ERROR,
					(EPROTO,
					 gtm_errmsg("Message does not contain valid GXID")));
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, (char *)&gxid[0], sizeof(GlobalTransactionId));
		gtm_pq_sendbytes(&buf, (char *)&status[0], 4);
		gtm_pq_endmessage(myport, &buf);
...
ProcessGetGIDDataTransactionCommand(Port *myport, StringInfo message)
{
	...
	/* receive GID */
	gidlen = gtm_pq_getmsgint(message, sizeof (GTM_StrLen));
	memcpy(gid, (char *)gtm_pq_getmsgbytes(message, gidlen), gidlen);
	gid[gidlen] = '\0';
	...
	if (prepared_txn == InvalidTransactionHandle)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to get GID Data for prepared transaction")));
	...
	if (GTM_GetGIDData(prepared_txn, &prepared_gxid, &nodestring) != STATUS_OK)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to get the information of prepared transaction")));
...
retry:
	...
	gtm_pq_sendbytes(&buf, (char *)&prepared_gxid, sizeof(GlobalTransactionId));

	/* Node string list */
	if (nodestring)
	{
		pq_sendint(&buf, strlen(nodestring), 4);
		gtm_pq_sendbytes(&buf, nodestring, strlen(nodestring));
	}
	else
		pq_sendint(&buf, 0, 4);
...
ProcessGXIDListCommand(Port *myport, StringInfo message)
{
	...
	if (Recovery_IsStandby())
		ereport(ERROR,
			(EPERM,
			 gtm_errmsg("Operation not permitted under the standby mode.")));
	...
	gtm_pq_sendbytes(&buf, data, actlen);			/* serialized GTM_Transactions */
	gtm_pq_endmessage(myport, &buf);
...
ProcessCommitTransactionCommandMulti(Port *myport, StringInfo message, bool is_backup)
{
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, (char *)&txn_count, sizeof(txn_count));
		gtm_pq_sendbytes(&buf, (char *)status, sizeof(int) * txn_count);
		gtm_pq_endmessage(myport, &buf);
...
ProcessStartPreparedTransactionCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	nodelen = gtm_pq_getmsgint(message, sizeof (GTM_StrLen));
	memcpy(nodestring, (char *)gtm_pq_getmsgbytes(message, nodelen), nodelen);
	...
	if (GTM_StartPreparedTransaction(txn, gid, nodestring) != STATUS_OK)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to prepare the transaction")));
...
ProcessGetGXIDTransactionCommand(Port *myport, StringInfo message)
{
	...
	data = gtm_pq_getmsgbytes(message, sizeof (txn));
	if (data == NULL)
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("Message does not contain valid Transaction Handle")));
	...
	if (GlobalTransactionIdIsValid(gxid))
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Failed to get the transaction id")));
