				  ...
				  void *(* startroutine)(void *))
{
							  ...
							  thrinfo)))
	{
		ereport(LOG,
				(err,
				 gtm_errmsg("Failed to create a new thread: error %s", strerror(err))));
...
GTM_ThreadCleanup(void *argp)
{
	 ...
	 */
	Gtm_StreamClose(thrinfo->thr_conn->con_port->sock);
	...
	if (thrinfo->thr_conn->con_port->node_name != NULL)
		 ...
		 */
		gtm_pfree(thrinfo->thr_conn->con_port->node_name);
