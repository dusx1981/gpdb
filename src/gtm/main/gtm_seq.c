...
seq_add_seqinfo(GTM_SeqInfo *seqinfo)
{
	...
	gtm_foreach(elem, bucket->shb_list)
	{
		...
		if (seq_keys_equal(curr_seqinfo->gs_key, seqinfo->gs_key))
		{
			...
			ereport(LOG,
					(EEXIST,
					 gtm_errmsg("Sequence with the given key already exists")));
			return EEXIST;
		}
	}
...
seq_copy_key(GTM_SequenceKey key)
{
	 ...
	 * contextes.
	 */
	retkey = (GTM_SequenceKey) Gtm_MemoryContextAlloc(TopMostMemoryContext,
												  sizeof(GTM_SequenceKeyData) +
														key->gsk_keylen);

	if (retkey == NULL)
		ereport(ERROR, (ENOMEM, gtm_errmsg("Out of memory")));
			...
			bool cycle)
{
	...
	if (SEQVAL_IS_VALID(maxval))
	{
		if (maxval < seqinfo->gs_min_value)
			ereport(ERROR,
					(ERANGE,
					 gtm_errmsg("Max value must be greater than min value")));
	...
	if (SEQVAL_IS_VALID(startval))
	{
		if (startval < seqinfo->gs_min_value)
			ereport(ERROR,
					(ERANGE,
					 gtm_errmsg("Start value must be greater than or equal to the min value")));

		if (startval > seqinfo->gs_max_value)
			ereport(ERROR,
					(ERANGE,
					 gtm_errmsg("Start value must be less than or equal to the max value")));
				 ...
				 bool is_restart)
{
	...
	if (seqinfo == NULL)
	{
		ereport(LOG,
				(EINVAL,
				 gtm_errmsg("The sequence with the given key does not exist")));
		return EINVAL;
	}
...
GTM_SeqRename(GTM_SequenceKey seqkey, GTM_SequenceKey newseqkey)
{
	...
	newseqinfo->gs_last_values = (GTM_SeqLastVal *)
			Gtm_MemoryContextAlloc(TopMostMemoryContext,
	...
	if ((errcode = seq_add_seqinfo(newseqinfo))) /* a lock is taken here for the new sequence */
	{
		...
		gtm_pfree(newseqinfo->gs_key);
		gtm_pfree(newseqinfo);
		return errcode;
	}
				  ...
				  int coord_procid, GTM_Sequence *result)
{
	...
	if (seqinfo == NULL)
	{
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("sequence \"%s\" does not exist", seqkey->gsk_key)));
		return;
	}
	...
	if (!found)
		ereport(ERROR,
				(ERANGE,
				 gtm_errmsg("currval of sequence \"%s\" is not yet defined in this session",
						seqkey->gsk_key)));
...
#define INIT_LASTVALS 16
		...
		else
		{
			...
			seqinfo->gs_last_values = (GTM_SeqLastVal *)
					gtm_repalloc(seqinfo->gs_last_values,
	...
	else
	{
		if (SEQ_IS_ASCENDING(seqinfo))
		{
			...
			else
			{
				...
				ereport(LOG,
						(ERANGE,
						 gtm_errmsg("Sequence reached maximum value")));
				return ERANGE;
			}
		}
		else
...
GTM_SeqGetNext(GTM_SequenceKey seqkey)
{
	...
	else
	{
		...
		else
		{
			...
			ereport(LOG,
					(ERANGE,
					 gtm_errmsg("Sequence reached minimum value")));
			return InvalidSequenceValue;
		}
...
ProcessSequenceInitCommand(Port *myport, StringInfo message, bool is_backup)
{
	 ...
	 */
	seqkey.gsk_keylen = gtm_pq_getmsgint(message, sizeof (seqkey.gsk_keylen));
	seqkey.gsk_key = (char *)gtm_pq_getmsgbytes(message, seqkey.gsk_keylen);

	/*
	 * Read various sequence parameters
	 */
	memcpy(&increment, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));
	memcpy(&minval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));
	memcpy(&maxval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));
	memcpy(&startval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));

	cycle = gtm_pq_getmsgbyte(message);
	...
	if ((errcode = GTM_SeqOpen(&seqkey, increment, minval, maxval, startval, cycle)))
		ereport(ERROR,
				(errcode,
				 gtm_errmsg("Failed to open a new sequence")));
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, seqkey.gsk_key, seqkey.gsk_keylen);
...
ProcessSequenceAlterCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	memcpy(&startval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		sizeof (GTM_Sequence));
	memcpy(&lastval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		sizeof (GTM_Sequence));

	cycle = gtm_pq_getmsgbyte(message);
	is_restart = gtm_pq_getmsgbyte(message);
	...
	for (i = 0 ; i < SEQ_HASH_TABLE_SIZE ; i++)
	{
		...
		gtm_foreach(elem, b->shb_list)
		{
			...
			if (seq_count == seq_maxcount)
			{
				...
				newlist = (GTM_SeqInfo **) gtm_repalloc(seq_list, newcount * sizeof(GTM_SeqInfo *));
				/*
				...
				while (newlist == NULL)
				{
					...
					if (newcount <= seq_maxcount)
					{
						...
						ereport(ERROR,
								(ERANGE,
								 gtm_errmsg("Can not list all the sequences")));
					}
					newlist = (GTM_SeqInfo **) gtm_repalloc(seq_list, newcount * sizeof(GTM_SeqInfo *));
				}
	 ...
	 */
	{
		...
		for (i = 0 ; i < seq_count ; i++)
		{
			...
			if (seq_buflen > seq_maxlen)
			{
				...
				seq_buf = (char *)gtm_repalloc(seq_buf, seq_maxlen);
			}
			...
			gtm_pq_sendbytes(&buf, seq_buf, seq_buflen);
		}
	}
...
ProcessSequenceGetCurrentCommand(Port *myport, StringInfo message)
{
	...
	seqkey.gsk_key = (char *)gtm_pq_getmsgbytes(message, seqkey.gsk_keylen);
#ifdef XCP
	coord_namelen = gtm_pq_getmsgint(message, sizeof(coord_namelen));
	if (coord_namelen > 0)
		coord_name = (char *)gtm_pq_getmsgbytes(message, coord_namelen);
	else
		coord_name = NULL;
	coord_procid = gtm_pq_getmsgint(message, sizeof(coord_procid));
	...
	if (!SEQVAL_IS_VALID(seqval))
		ereport(ERROR,
				(ERANGE,
				 gtm_errmsg("Can not get current value of the sequence")));
#endif
	...
	pq_sendint(&buf, seqkey.gsk_keylen, 4);
	gtm_pq_sendbytes(&buf, seqkey.gsk_key, seqkey.gsk_keylen);
	gtm_pq_sendbytes(&buf, (char *)&seqval, sizeof (GTM_Sequence));
	gtm_pq_endmessage(myport, &buf);
...
ProcessSequenceGetNextCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	if (coord_namelen > 0)
		...
		coord_name = NULL;
	coord_procid = gtm_pq_getmsgint(message, sizeof(coord_procid));
	memcpy(&range, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, (char *)&seqval, sizeof (GTM_Sequence));
#ifdef XCP
		gtm_pq_sendbytes(&buf, (char *)&rangemax, sizeof (GTM_Sequence));
#endif
		gtm_pq_endmessage(myport, &buf);
...
ProcessSequenceSetValCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	memcpy(&nextval, gtm_pq_getmsgbytes(message, sizeof (GTM_Sequence)),
		   sizeof (GTM_Sequence));

	iscalled = gtm_pq_getmsgbyte(message);
	...
	if ((errcode = GTM_SeqSetVal(&seqkey, coord_name, coord_procid, nextval, iscalled)))
		ereport(ERROR,
				(errcode,
				 gtm_errmsg("Failed to set values of sequence")));
#else
...
ProcessSequenceResetCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	if ((errcode = GTM_SeqReset(&seqkey)))
		ereport(ERROR,
				(errcode,
				 gtm_errmsg("Can not reset the sequence")));
...
ProcessSequenceCloseCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	seqkey.gsk_keylen = gtm_pq_getmsgint(message, sizeof (seqkey.gsk_keylen));
	seqkey.gsk_key = (char *)gtm_pq_getmsgbytes(message, seqkey.gsk_keylen);
	memcpy(&seqkey.gsk_type, gtm_pq_getmsgbytes(message, sizeof (GTM_SequenceKeyType)),
	...
	if ((errcode = GTM_SeqClose(&seqkey)))
		ereport(ERROR,
				(errcode,
				 gtm_errmsg("Can not close the sequence")));
...
ProcessSequenceRenameCommand(Port *myport, StringInfo message, bool is_backup)
{
	...
	newseqkey.gsk_keylen = gtm_pq_getmsgint(message, sizeof (newseqkey.gsk_keylen));
	newseqkey.gsk_key = (char *)gtm_pq_getmsgbytes(message, newseqkey.gsk_keylen);
	...
	if ((errcode = GTM_SeqRename(&seqkey, &newseqkey)))
		ereport(ERROR,
				(errcode,
				 gtm_errmsg("Can not rename the sequence")));
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, newseqkey.gsk_key, newseqkey.gsk_keylen);
		gtm_pq_endmessage(myport, &buf);
			...
			for (j = 0; j < curr_seqinfo->gs_lastval_count; j++)
			{
						...
						lastval->gs_coord_procid == coord_procid)
				{
					...
					if (curr_seqinfo->gs_lastval_count == 0)
					{
						...
						gtm_pfree(curr_seqinfo->gs_last_values);
