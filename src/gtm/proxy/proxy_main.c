...
#define INVALID_RECONNECT_OPTION_MSG() \
	do{ \
		ereport(ERROR, (0, gtm_errmsg("Invalid Reconnect Option"))); \
	} while(0)
...
main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "h:i:p:n:D:l:s:t:")) != -1)
	{
		switch (opt)
		{
			...
			default:
				gtm_write_stderr("Try \"%s --help\" for more information.\n",
							 progname);
		}
	}
	...
	{
		gtm_write_stderr("GTM Proxy data directory must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr("GTM Proxy Node name must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr("GTM Proxy listen addresses must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr("GTM Proxy port number must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	...
	{
		gtm_write_stderr("GTM server listen address must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	...
	{
		gtm_write_stderr("GTM server port number must be specified\n");
		gtm_write_stderr("Try \"%s --help\" for more information.\n",
					 progname);
		exit(1);
	...
	if (optind < argc)
	{
		gtm_write_stderr("%s: invalid argument: \"%s\"\n",
	...
	if (ListenAddresses)
	{
		...
		if (strcmp(ListenAddresses, "*") == 0)
			status = Gtm_StreamServerPort(AF_UNSPEC, NULL,
									  (unsigned short) GTMProxyPortNumber,
									  ListenSocket, MAXLISTEN);
		else
			status = Gtm_StreamServerPort(AF_UNSPEC, ListenAddresses,
									  (unsigned short) GTMProxyPortNumber,
									  ListenSocket, MAXLISTEN);
		...
		if (status == STATUS_OK)
			...
			ereport(FATAL,
					(gtm_errmsg("could not create listen socket for \"%s\"",
							ListenAddresses)));
	}
	...
	if (ListenSocket[0] == -1)
		ereport(FATAL,
				(gtm_errmsg("no socket created for listening")));
...
ConnCreate(int serverFd)
{
	...
	if (!(port = (Port *) calloc(1, sizeof(Port))))
	{
		ereport(LOG,
				(ENOMEM,
				 gtm_errmsg("out of memory")));
		return NULL;
	}

	if (Gtm_StreamConnection(serverFd, port) != STATUS_OK)
	{
		if (port->sock >= 0)
			Gtm_StreamClose(port->sock);
		ConnFree(port);
	...
	for (;;)
	{
		...
		if (selres < 0)
		{
			if (errno != EINTR && errno != EWOULDBLOCK)
			{
				ereport(DEBUG1,
						(EACCES,
						 gtm_errmsg("select() failed in main thread: %m")));
				return STATUS_ERROR;
			}
		}
...
GTMProxy_ThreadMain(void *argp)
{
			...
			for (ii = 0; ii < thrinfo->thr_conn_count; ii++)
			{
				...
				else
				{
					...
					if (conninfo->con_pending_msg != MSG_TYPE_INVALID)
					{
						...
						Gtm_EmitErrorReport(conninfo->con_port);
					}
				}
			}
		}
		else
			Gtm_EmitErrorReport(NULL);
	...
	for (;;)
	{
		...
		Gtm_MemoryContextResetAndDeleteChildren(MessageContext);
...
setjmp_again:
		...
		/*
		 * Just reset the input buffer to avoid repeated palloc/gtm_pfrees
		 *
		...
		StringInfo input_message)
{
	...
	mtype = gtm_pq_getmsgint(input_message, sizeof (GTM_MessageType));
	...
	switch (mtype)
	{
		...
		default:
			ereport(FATAL,
					(EPROTO,
					 gtm_errmsg("invalid frontend message type %d",
							mtype)));
	}
	...
	switch (cmdinfo->ci_mtype)
	{
		case MSG_TXN_BEGIN_GETGXID:
			...
			if (res->gr_status == GTM_RESULT_OK)
			{
				...
				gtm_pq_beginmessage(&buf, 'S');
				pq_sendint(&buf, TXN_BEGIN_GETGXID_RESULT, 4);
				gtm_pq_sendbytes(&buf, (char *)&gxid, sizeof (GlobalTransactionId));
				gtm_pq_sendbytes(&buf, (char *)&timestamp, sizeof (GTM_Timestamp));
				gtm_pq_endmessage(cmdinfo->ci_conn->con_port, &buf);
				pq_flush(cmdinfo->ci_conn->con_port);
			}
			else
			{
				gtm_pq_beginmessage(&buf, 'E');
				gtm_pq_sendbytes(&buf, res->gr_proxy_data, res->gr_msglen);
				gtm_pq_endmessage(cmdinfo->ci_conn->con_port, &buf);
				pq_flush(cmdinfo->ci_conn->con_port);
			}
		...
		case MSG_TXN_COMMIT_MULTI:
			...
			if (res->gr_resdata.grd_txn_rc_multi.status[cmdinfo->ci_res_index] == STATUS_OK)
			{
				...
				gtm_pq_sendbytes(&buf, &txn_count, sizeof (int));
				gtm_pq_sendbytes(&buf, &status, sizeof (int));
				gtm_pq_endmessage(cmdinfo->ci_conn->con_port, &buf);
				pq_flush(cmdinfo->ci_conn->con_port);
			}
			else
			{
				ReleaseCmdBackup(cmdinfo);
				ereport(ERROR2, (EINVAL, gtm_errmsg("Transaction commit failed")));
			}
			cmdinfo->ci_conn->con_pending_msg = MSG_TYPE_INVALID;
			ReleaseCmdBackup(cmdinfo);
			break;
		...
		case MSG_TXN_ROLLBACK:
			...
			if (res->gr_resdata.grd_txn_rc_multi.status[cmdinfo->ci_res_index] == STATUS_OK)
			{
				...
				gtm_pq_sendbytes(&buf, (char *)&cmdinfo->ci_data.cd_rc.gxid, sizeof (GlobalTransactionId));
				gtm_pq_endmessage(cmdinfo->ci_conn->con_port, &buf);
				pq_flush(cmdinfo->ci_conn->con_port);
			}
			else
		...
		case MSG_SNAPSHOT_GET_MULTI:
		   	...
		   	if ((status == STATUS_OK) || (status == STATUS_NOT_FOUND))
			{
				...
				gtm_pq_sendbytes(&buf, (char *)&txn_count, sizeof (txn_count));
				gtm_pq_sendbytes(&buf, (char *)&status, sizeof (status));
				gtm_pq_sendbytes(&buf, (char *)&res->gr_snapshot.sn_xmin, sizeof (GlobalTransactionId));
				gtm_pq_sendbytes(&buf, (char *)&res->gr_snapshot.sn_xmax, sizeof (GlobalTransactionId));
				gtm_pq_sendbytes(&buf, (char *)&res->gr_snapshot.sn_recent_global_xmin, sizeof (GlobalTransactionId));
				pq_sendint(&buf, res->gr_snapshot.sn_xcnt, sizeof (int));
				gtm_pq_sendbytes(&buf, (char *)res->gr_snapshot.sn_xip,
			...
			{
				ReleaseCmdBackup(cmdinfo);
				ereport(ERROR2, (EINVAL, gtm_errmsg("snapshot request failed")));
			}
			cmdinfo->ci_conn->con_pending_msg = MSG_TYPE_INVALID;
			ReleaseCmdBackup(cmdinfo);
			break;
...
ReadCommand(GTMProxy_ConnectionInfo *conninfo, StringInfo inBuf)
{
	...
	if (!anyBackup)
	{
		qtype = gtm_pq_getbyte(conninfo->con_port);
	...
	if (qtype == EOF)			/* frontend disconnected */
	{
		ereport(COMMERROR,
				(EPROTO,
				 gtm_errmsg("unexpected EOF on client connection")));
		return qtype;
	}
	...
	switch (qtype)
	{
		...
		default:
			...
			ereport(ERROR,
					(EPROTO,
					 gtm_errmsg("invalid frontend message type %d", qtype)));
	...
	if (!anyBackup)
	{
		if (gtm_pq_getmessage(conninfo->con_port, inBuf, 0))
	...
	switch(mtype)
	{
		case MSG_NODE_REGISTER:
		{
									...
									NI_NUMERICSERV))
			{
				...
				if (ret)
					ereport(WARNING,
							(gtm_errmsg_internal("gtm_getnameinfo_all() failed")));
			}

			/* Get the node type */
			memcpy(&cmd_data.cd_reg.type, gtm_pq_getmsgbytes(message, sizeof (GTM_PGXCNodeType)), sizeof (GTM_PGXCNodeType));

			/* Then obtain the node name */
			len = gtm_pq_getmsgint(message, sizeof(GTM_StrLen));
			cmd_data.cd_reg.nodename = palloc(len + 1);
			memcpy(cmd_data.cd_reg.nodename, (char *)gtm_pq_getmsgbytes(message, len), len);
			...
			memcpy(cmd_data.cd_reg.ipaddress, (char *)gtm_pq_getmsgbytes(message, len), len);
			cmd_data.cd_reg.ipaddress[len] = '\0';

			/* Then the next is the port number */
			memcpy(&cmd_data.cd_reg.port,
				   gtm_pq_getmsgbytes(message,
			...
			memcpy(cmd_data.cd_reg.gtm_proxy_nodename, (char *)gtm_pq_getmsgbytes(message, len), len);
			cmd_data.cd_reg.gtm_proxy_nodename[len] = '\0';

			/* get data folder data */
			len = gtm_pq_getmsgint(message, sizeof (int));
			cmd_data.cd_reg.datafolder = palloc(len + 1);
			memcpy(cmd_data.cd_reg.datafolder, (char *)gtm_pq_getmsgbytes(message, len), len);
			cmd_data.cd_reg.datafolder[len] = '\0';

			/* Now we have one more data to waste, "status" */
			cmd_data.cd_reg.status = gtm_pq_getmsgint(message, sizeof(GTM_PGXCNodeStatus));
			gtm_pq_getmsgend(message);
										  ...
										  conninfo->con_port->sock))
			{
				ereport(ERROR,
						(EINVAL,
						 gtm_errmsg("Failed to Register node")));
			}
		...
		case MSG_NODE_UNREGISTER:
		{
			...
			memcpy(nodename, gtm_pq_getmsgbytes(message, len), len);
								...
								conninfo->con_port->sock))
			{
				ereport(ERROR,
						(EINVAL,
						 gtm_errmsg("Failed to Unregister node")));
			}
#endif
			MemoryContextSwitchTo(oldContext);
		...
		GTM_MessageType mtype, StringInfo message)
{
	...
	{
		case MSG_TXN_BEGIN_GETGXID:
			cmd_data.cd_beg.iso_level = gtm_pq_getmsgint(message, sizeof (GTM_IsolationLevel));
			cmd_data.cd_beg.rdonly = gtm_pq_getmsgbyte(message);
			GTMProxy_CommandPending(conninfo, mtype, cmd_data);
			break;

		case MSG_TXN_COMMIT_MULTI:
			{
				int txn_count = gtm_pq_getmsgint(message, sizeof (int));
				Assert (txn_count == 1);
			}
			/* fall through */
		case MSG_TXN_ROLLBACK:
			{
				const char *data = gtm_pq_getmsgbytes(message,
						sizeof (GlobalTransactionId));
				if (data == NULL)
					ereport(ERROR,
							(EPROTO,
							 gtm_errmsg("Message does not contain valid GXID")));
		...
		GTM_MessageType mtype, StringInfo message)
{
	...
	unreadmsg = gtm_pq_getmsgbytes(message, unreadmsglen);
...
GTMProxy_HandshakeConnection(GTMProxy_ConnectionInfo *conninfo)
{
	...
	startup_type = gtm_pq_getbyte(conninfo->con_port);

	if (startup_type != 'A')
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("Expecting a startup message, but received %c",
					 startup_type)));
	...
	if (gtm_pq_getmessage(conninfo->con_port, &inBuf, 0))
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("Expecting PGXC Node ID, but received EOF")));

	memcpy(&sp,
		   gtm_pq_getmsgbytes(&inBuf, sizeof (GTM_StartupPacket)),
		   sizeof (GTM_StartupPacket));
	gtm_pq_getmsgend(&inBuf);
	 ...
	 */
	gtm_pq_beginmessage(&buf, 'R');
	pq_sendint(&buf, 0, 4);
	gtm_pq_endmessage(conninfo->con_port, &buf);
...
GTMProxy_HandleDisconnect(GTMProxy_ConnectionInfo *conninfo, GTM_Conn *gtm_conn)
{
	...
	if (conninfo->con_port->sock > 0)
		Gtm_StreamClose(conninfo->con_port->sock);
...
GTMProxy_ProcessPendingCommands(GTMProxy_ThreadInfo *thrinfo)
{
	...
	gtm_foreach (elem, thrinfo->thr_pending_commands[MSG_BACKEND_DISCONNECT])
	{
		ereport(COMMERROR,
				(EPROTO,
				 gtm_errmsg("cleaning up client disconnection")));
		cmdinfo = (GTMProxy_CommandInfo *)gtm_lfirst(elem);
...
retry:
	if (stat(GTMProxyDataDir, &stat_buf) != 0)
	{
		if (errno == ENOENT)
		{
			if (mkdir(GTMProxyDataDir, 0700) != 0)
			{
				ereport(FATAL,
						(errno,
						 gtm_errmsg("failed to create the directory \"%s\"",
							 GTMProxyDataDir)));
			}
			goto retry;
		}
			...
			ereport(FATAL,
					(EPERM,
				 gtm_errmsg("could not read permissions of directory \"%s\": %m",
						GTMProxyDataDir)));
	}
	...
	if (!S_ISDIR(stat_buf.st_mode))
		ereport(FATAL,
				(EINVAL,
				 gtm_errmsg("specified data directory \"%s\" is not a directory",
						GTMProxyDataDir)));
	...
	if (stat_buf.st_uid != geteuid())
		ereport(FATAL,
				(EINVAL,
				 gtm_errmsg("data directory \"%s\" has wrong ownership",
						GTMProxyDataDir),
				 gtm_errhint("The server must be started by the user that owns the data directory.")));
#endif
}
...
SetDataDir(void)
{
	...
	if (!new)
		ereport(FATAL,
				(errno,
				 gtm_errmsg("failed to set the data directory \"%s\"",
						GTMProxyDataDir)));
...
ChangeToDataDir(void)
{
	if (chdir(GTMProxyDataDir) < 0)
		ereport(FATAL,
				(EINVAL,
				 gtm_errmsg("could not change directory to \"%s\": %m",
						GTMProxyDataDir)));
}
...
CreateLockFile(const char *filename, const char *refName)
{
	...
	for (ntries = 0;; ntries++)
	{
		...
		if ((errno != EEXIST && errno != EACCES) || ntries > 100)
			ereport(FATAL,
					(EINVAL,
					 gtm_errmsg("could not create lock file \"%s\": %m",
							filename)));
		...
		if (fd < 0)
		{
			...
			ereport(FATAL,
					(EINVAL,
					 gtm_errmsg("could not open lock file \"%s\": %m",
							filename)));
		}
		if ((len = read(fd, buffer, sizeof(buffer) - 1)) < 0)
			ereport(FATAL,
					(EINVAL,
					 gtm_errmsg("could not read lock file \"%s\": %m",
							filename)));
		close(fd);
			...
			)
		{
				...
				(errno != ESRCH && errno != EPERM))
			{
				...
				ereport(FATAL,
						(EINVAL,
						 gtm_errmsg("lock file \"%s\" already exists",
								filename),
						  gtm_errhint("Is another GTM proxy (PID %d) running in data directory \"%s\"?",
		...
		if (unlink(filename) < 0)
			ereport(FATAL,
					(EACCES,
					 gtm_errmsg("could not remove old lock file \"%s\": %m",
							filename),
					 gtm_errhint("The file seems accidentally left over, but "
	...
	if (write(fd, buffer, strlen(buffer)) != strlen(buffer))
	{
		...
		ereport(FATAL,
				(EACCES,
				 gtm_errmsg("could not write lock file \"%s\": %m", filename)));
	}
	if (close(fd))
	{
		int			save_errno = errno;
