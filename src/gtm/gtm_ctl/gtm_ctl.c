...
pg_malloc(size_t size)
{
	...
	if (!result)
	{
		gtm_write_stderr(_("%s: out of memory\n"), progname);
		exit(1);
	}
	return result;
}
...
get_pgpid(void)
{
	...
	if (pidf == NULL)
	{
		...
		{
			gtm_write_stderr(_("%s: could not open PID file \"%s\": %s\n"),
	...
	{
		gtm_write_stderr(_("%s: invalid data in PID file \"%s\"\n"),
					 progname, pid_file);
		exit(1);
	}
	fclose(pidf);
...
start_gtm(void)
{
	...
	{
		gtm_write_stderr("gtm command exceeds max size");
		exit(1);
	}
...
read_gtm_opts(void)
{
	if (gtm_opts == NULL)
	{
		...
		if (ctl_command == RESTART_COMMAND)
		{
			...
			if (optlines == NULL)
			{
				gtm_write_stderr(_("%s: could not read file \"%s\"\n"), progname, gtmopts_file);
				exit(1);
			}
			else if (optlines[0] == NULL || optlines[1] != NULL)
			{
				gtm_write_stderr(_("%s: option file \"%s\" must have exactly one line\n"),
							 progname, gtmopts_file);
				exit(1);
			}
			else
...
do_start(void)
{
	...
	if (ctl_command != RESTART_COMMAND)
	{
		old_pid = get_pgpid();
		if (old_pid != 0)
			gtm_write_stderr(_("%s: another server might be running; "
	...
	if (exitcode != 0)
	{
		gtm_write_stderr(_("%s: could not start server: exit code was %d\n"),
					 progname, exitcode);
		exit(1);
	}
	...
	if (old_pid != 0)
	{
		...
		if (pid == old_pid)
		{
			gtm_write_stderr(_("%s: could not start server\n"
...
do_stop(void)
{
	...
	{
		gtm_write_stderr(_("%s: PID file \"%s\" does not exist\n"), progname, pid_file);
		gtm_write_stderr(_("Is server running?\n"));
		exit(1);
	}
	else if (pid < 0)			/* standalone backend, not gtm */
	{
		pid = -pid;
		gtm_write_stderr(_("%s: cannot stop server; "
	...
	{
		gtm_write_stderr(_("%s: could not send stop signal (PID: %ld): %s\n"), progname, pid,
					 strerror(errno));
		exit(1);
	}
	...
	else
	{
		...
		if (pid != 0)			/* pid file still exists */
		{
			...
			gtm_write_stderr(_("%s: server does not shut down\n"), progname);
			exit(1);
		}
		print_msg(_(" done\n"));
...
do_promote(void)
{
	...
	{
		pid = -pid;
		gtm_write_stderr(_("%s: cannot promote server; "
					   "single-user server is running (PID: %ld)\n"),
					 progname, pid);
		exit(1);
	}
	...
	{
		gtm_write_stderr(_("%s: could not send promote signal (PID: %ld): %s\n"), progname, pid,
					 strerror(errno));
		exit(1);
	}
}
...
do_reconnect(void)
{
...
#ifdef GTM_SBY_DEBUG
	gtm_write_stderr("Reconnecting to new GTM ... DEBUG MODE.");
#endif
	...
	{
		gtm_write_stderr(_("%s: only gtm_proxy can accept reconnect command\n"), progname);
		exit(1);
	}
	pid = get_pgpid();
	...
	{
		gtm_write_stderr(_("%s: No memory available.\n"), progname);
		exit(1);
	}
	...
	{
		gtm_write_stderr(_("%s: Cannot open reconnect point file %s\n"), progname, reconnect_point_file_nam);
		exit(1);
	}
...
do_restart(void)
{
	...
	if (pid == 0)				/* no pid file */
	{
		gtm_write_stderr(_("%s: PID file \"%s\" does not exist\n"),
					 progname, pid_file);
		gtm_write_stderr(_("Is server running?\n"));
		gtm_write_stderr(_("starting server anyway\n"));
		do_start();
		return;
	}
	else if (pid < 0)			/* standalone backend, not gtm */
	{
		...
		{
			gtm_write_stderr(_("%s: cannot restart server; "
						   "single-user server is running (PID: %ld)\n"),
						 progname, pid);
			gtm_write_stderr(_("Please terminate the single-user server and try again.\n"));
			exit(1);
		}
	}
	...
	{
		gtm_write_stderr(_("%s: old server process (PID: %ld) seems to be gone\n"),
					 progname, pid);
		gtm_write_stderr(_("starting server anyway\n"));
	}
...
do_status(void)
{
	...
	else if (pid < 0)			/* standalone backend, not gtm */
	{
		...
		if (gtm_is_alive((pid_t) pid))
		{
			gtm_write_stderr(_("%s: cannot get server status; "
						   "single-user server is running (PID: %ld)\n"),
						 progname, pid);
			gtm_write_stderr(_("Please terminate the single-user server and try again.\n"));
			exit(1);
	...
	gtm_write_stderr(_("%s: no server running\n"), progname);
	exit(1);
}
...
do_advice(void)
{
	gtm_write_stderr(_("Try \"%s --help\" for more information.\n"), progname);
}
...
set_mode(char *modeopt)
{
	...
	{
		gtm_write_stderr(_("%s: unrecognized shutdown mode \"%s\"\n"), progname, modeopt);
		do_advice();
		exit(1);
	}
}
...
main(int argc, char **argv)
{
	...
	if (geteuid() == 0)
	{
		gtm_write_stderr(_("%s: cannot be run as root\n"
	...
	while (optind < argc)
	{
		while ((c = getopt(argc, argv, "D:i:l:m:o:p:t:wWZ:")) != -1)
		{
			switch (c)
			{
				...
				case 'Z':
					...
					{
						gtm_write_stderr(_("%s: %s launch name set not correct\n"), progname, gtm_app);
						do_advice();
						exit(1);
					}
					break;
		...
		if (optind < argc)
		{
			...
			{
				gtm_write_stderr(_("%s: too many command-line arguments (first is \"%s\")\n"), progname, argv[optind]);
				do_advice();
				exit(1);
			}
			...
			{
				gtm_write_stderr(_("%s: unrecognized operation mode \"%s\"\n"),
							 progname, argv[optind]);
				do_advice();
				exit(1);
			}
	...
	if (ctl_command == NO_COMMAND)
	{
		gtm_write_stderr(_("%s: no operation specified\n"), progname);
		do_advice();
		exit(1);
	}
	...
	if (!gtm_data)
	{
		gtm_write_stderr("%s: no GTM/GTM Proxy directory specified \n",
					 progname);
		do_advice();
		exit(1);
	}
	...
	if (!gtm_app)
	{
		gtm_write_stderr("%s: no launch option not specified\n",
					 progname);
		do_advice();
		exit(1);
	}
	...
	if (strcmp(gtm_app,"gtm_proxy") != 0 &&
		...
		strcmp(gtm_app,"gtm") != 0)
	{
		gtm_write_stderr(_("%s: launch option incorrect\n"),
						progname);
		do_advice();
		exit(1);
	}
