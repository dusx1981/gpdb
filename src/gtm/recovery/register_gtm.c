...
ProcessPGXCNodeRegister(Port *myport, StringInfo message, bool is_backup)
{
	...
	/* Read Node Type */
	memcpy(&type, gtm_pq_getmsgbytes(message, sizeof (GTM_PGXCNodeType)),
	...
	if (len >= NI_MAXHOST)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Invalid name length.")));

	memcpy(node_name, (char *)gtm_pq_getmsgbytes(message, len), len);
	node_name[len] = '\0';

	 /* Read Host name */
	len = gtm_pq_getmsgint(message, sizeof (int));
	memcpy(remote_host, (char *)gtm_pq_getmsgbytes(message, len), len);
	remote_host[len] = '\0';
	ipaddress = remote_host;

	/* Read Port Number */
	memcpy(&port, gtm_pq_getmsgbytes(message, sizeof (GTM_PGXCNodePort)),
	...
	if (len >= NI_MAXHOST)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Invalid proxy name length.")));
	memcpy(proxyname, (char *)gtm_pq_getmsgbytes(message, len), len);
	proxyname[len] = '\0';
	...
	memcpy(datafolder, (char *)gtm_pq_getmsgbytes(message, len), len);
	datafolder[len] = '\0';

	status = gtm_pq_getmsgint(message, sizeof (GTM_PGXCNodeStatus));
	...
	if (!is_backup)
	{
		...
		gtm_pq_sendbytes(&buf, (char *)&type, sizeof(GTM_PGXCNodeType));
		/* Node name length */
		pq_sendint(&buf, strlen(node_name), 4);
		/* Node name (var-len) */
		gtm_pq_sendbytes(&buf, node_name, strlen(node_name));
...
ProcessPGXCNodeUnregister(Port *myport, StringInfo message, bool is_backup)
{
	...
	if (len >= NI_MAXHOST)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Invalid node name length")));
	memcpy(node_name, (char *)gtm_pq_getmsgbytes(message, len), len);
	node_name[len] = '\0';
...
ProcessPGXCNodeList(Port *myport, StringInfo message)
{
	...
	for (i = 0; i < num_node; i++)
	{
		...
		gtm_pq_sendbytes(&buf, s_data[i], s_datalen[i]);
	}
...
ProcessPGXCRegisterSession(Port *myport, StringInfo message)
{
	...
	len = gtm_pq_getmsgint(message, sizeof(len));
	if (len >= SP_NODE_NAME)
		ereport(ERROR,
				(EINVAL,
				 gtm_errmsg("Invalid name length.")));

	memcpy(coord_name, (char *)gtm_pq_getmsgbytes(message, len), len);
	coord_name[len] = '\0';

	coord_procid = gtm_pq_getmsgint(message, sizeof(coord_procid));

	coord_backendid = gtm_pq_getmsgint(message, sizeof(coord_backendid));
