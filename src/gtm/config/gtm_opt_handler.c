...
#undef fprintf
#define fprintf(file, fmt, msg)  ereport(ERROR, (gtm_errmsg_internal("%s", msg)))
...
ProcessConfigFile(GtmOptContext context)
{
	...
	for (i = 0; i < num_gtm_opt_variables; i++)
	{
		...
		if (gconf->context < GTMC_SIGHUP)
		{
			...
			if (isStartUp)
			{
				gtm_write_stderr("parameter \"%s\" cannot be changed without restarting the server",
							 gconf->name);
			}
			else
			{
				ereport(elevel,
						(0,
						 gtm_errmsg("parameter \"%s\" cannot be changed without restarting the server",
								gconf->name)));
			}
			continue;
		}
		...
		if (context == GTMC_SIGHUP)
		{
			if (isStartUp)
			{
				gtm_write_stderr("parameter \"%s\" removed from configuration file, reset to default\n",
							 gconf->name);
			}
			else
			{
				ereport(elevel,
						(gtm_errmsg("parameter \"%s\" removed from configuration file, reset to default",
								gconf->name)));
			}
		}
	}
								 ...
								 GTMC_S_FILE, true))
		{
			...
			if (pre_value)
			{
				...
				if (strcmp(pre_value, post_value) != 0)
				{
					if (isStartUp)
					{
						gtm_write_stderr("parameter \"%s\" changed to \"%s\"\n",
									 item->name, item->value);
					}
					else
					{
						ereport(elevel,
								(gtm_errmsg("parameter \"%s\" changed to \"%s\"",
										item->name, item->value)));
					}
				}
			}
				...
				ConfigVariable **tail_p)
{
	...
	if (depth > 10)
	{
		if (isStartUp)
		{
			gtm_write_stderr("could not open configuration file \"%s\": maximum nesting depth exceeded\n",
						 config_file);
		}
		else
		{
			ereport(elevel,
					(0,
					 gtm_errmsg("could not open configuration file \"%s\": maximum nesting depth exceeded",
							config_file)));
		}
		return false;
	}
	...
	if (!fp)
	{
		if (isStartUp)
		{
			gtm_write_stderr("could not open configuration file \"%s\": %m\n",
						 config_file);
		}
		else
		{
			ereport(elevel,
					(0,
					 gtm_errmsg("could not open configuration file \"%s\": %m",
							config_file)));
		}
		return false;
	}
			  ...
			  ConfigVariable **head_p, ConfigVariable **tail_p)
{
 ...
 parse_error:
	if (token == GTMOPT_EOL || token == 0)
	{
		if (isStartUp)
		{
			gtm_write_stderr("syntax error in file \"%s\" line %u, near end of line\n",
		...
		else
		{
			ereport(elevel,
					(0,
					 gtm_errmsg("syntax error in file \"%s\" line %u, near end of line",
							config_file, ConfigFileLineno - 1)));
		}
	}
	else
	{
		if (isStartUp)
		{
			gtm_write_stderr("syntax error in file \"%s\" line %u, near token \"%s\"\n",
		...
		else
		{
			ereport(elevel,
					(0,
					 gtm_errmsg("syntax error in file \"%s\" line %u, near token \"%s\"",
							config_file, ConfigFileLineno, GTMOPT_yytext)));
		}
	}
	OK = false;
...
gtm_opt_malloc(int elevel, size_t size)
{
	...
	if (data == NULL)
	{
		if (isStartUp)
		{
			gtm_write_stderr("out of memory\n");
		}
		else
		{
			ereport(elevel,
...
SelectConfigFiles(const char *userDoption, const char *progname)
{
	...
	{
		gtm_write_stderr("%s does not know where to find the server configuration file.\n"
	...
	{
		gtm_write_stderr("%s cannot access the server configuration file \"%s\": %s\n",
...
push_old_value(struct config_generic * gconf)
{
	 ...
	 */
	stack = (GtmOptStack *) Gtm_Gtm_MemoryContextAllocZero(TopMemoryContext,
												sizeof(GtmOptStack));
				  ...
				  bool changeVal)
{
	...
	if (record == NULL)
	{
		if (isStartUp)
		{
			gtm_write_stderr("unrecognized configuration parameter \"%s\"\n", name);
		}
		else
		{
			ereport(elevel,
					(0,
					 gtm_errmsg("unrecognized configuration parameter \"%s\"", name)));
		}
		return false;
	}
	...
	switch (record->context)
	{
		...
		case GTMC_SIGHUP:
			if (context != GTMC_SIGHUP && context != GTMC_STARTUP)
			{
				if (isStartUp)
				{
					gtm_write_stderr("parameter \"%s\" cannot be changed now\n",
								 name);
				}
				else
				{
					ereport(elevel,
							(0,
							 gtm_errmsg("parameter \"%s\" cannot be changed now",
									name)));
				}
				return false;
			}
		...
		default:
			if (isStartUp)
			{
				gtm_write_stderr("GtmOptContext invalid (%d)\n",
							 context);
			}
			else
			{
				ereport(elevel,
						(0,
						 gtm_errmsg("GtmOptContext invalid (%d)",
								context)));
			}
			return false;
	}
	...
	if (record->source > source)
	{
		if (changeVal && !makeDefault)
		{
			if (isStartUp)
			{
				gtm_write_stderr("\"%s\": setting ignored because previous source is higher priority\n",
							 name);
			}
			else
			{
		...
		case GTMC_BOOL:
			{
				...
				if (value)
				{
					if (!gtm_opt_parse_bool(value, &newval))
					{
						if (isStartUp)
						{
							gtm_write_stderr("parameter \"%s\" requires a Boolean value\n",
										 name);
						}
						else
						{
							ereport(elevel,
									(0,
									 gtm_errmsg("parameter \"%s\" requires a Boolean value",
											name)));
						}
						return false;
					}
		...
		case GTMC_INT:
			{
				...
				if (value)
				{
					...
					if (!parse_int(value, &newval, conf->gen.flags, &hintmsg))
					{
						if (isStartUp)
						{
							gtm_write_stderr("invalid value for parameter \"%s\": \"%s\"\n",
										 name, value);
						}
						else
						{
							ereport(elevel,
									(0,
									 gtm_errmsg("invalid value for parameter \"%s\": \"%s\"",
											name, value),
									 hintmsg ? gtm_errhint("%s", _(hintmsg)) : 0));
						}
						return false;
					}
					if (newval < conf->min || newval > conf->max)
					{
						if (isStartUp)
						{
							gtm_write_stderr("%d is outside the valid range for parameter \"%s\" (%d .. %d)\n",
						...
						else
						{
							ereport(elevel,
									(0,
									 gtm_errmsg("%d is outside the valid range for parameter \"%s\" (%d .. %d)",
											newval, name, conf->min, conf->max)));
						}
						return false;
					}
		...
		case GTMC_REAL:
			{
				...
				if (value)
				{
					if (!parse_real(value, &newval))
					{
						if (isStartUp)
						{
							gtm_write_stderr("parameter \"%s\" requires a numeric value\n",
										 name);
						}
						else
						{
							ereport(elevel,
									(0,
									 gtm_errmsg("parameter \"%s\" requires a numeric value",
											name)));
						}
						return false;
					}
					if (newval < conf->min || newval > conf->max)
					{
						if (isStartUp)
						{
							gtm_write_stderr("%g is outside the valid range for parameter \"%s\" (%g .. %g)\n",
										 newval, name, conf->min, conf->max);
						}
						else
						{
							ereport(elevel,
									(0,
									 gtm_errmsg("%g is outside the valid range for parameter \"%s\" (%g .. %g)",
											newval, name, conf->min, conf->max)));
						}
						return false;
					}
		...
		case GTMC_ENUM:
			{
				...
				if (value)
				{
					if (!config_enum_lookup_by_name(conf, value, &newval))
					{
						...
						if (isStartUp)
						{
							gtm_write_stderr("invalid value for parameter \"%s\": \"%s\". %s\n",
										 name, value, hintmsg);
						}
						else
						{
	...
	for (i = 0; i < num_gtm_opt_variables; i++)
	{
			...
			var->name[classLen] == GTMOPT_QUALIFIER_SEPARATOR)
		{
			if (isStartUp)
				...
				ereport(WARNING,
						(0,
						 gtm_errmsg("unrecognized configuration parameter \"%s\"",
								var->name)));
		}
	}
}
...
gtm_assign_tcp_keepalives_idle(int newval, void *extra)
{
	 ...
	 */
#if 0
	(void) gtm_pq_setkeepalivesidle(newval, MyProcPort);
#else
...
#if 0
	snprintf(nbuf, sizeof(nbuf), "%d", gtm_pq_getkeepalivesidle(MyProcPort));
#else
...
gtm_assign_tcp_keepalives_interval(int newval, void *extra)
{
	/* See comments in assign_tcp_keepalives_idle */
#if 0
	(void) gtm_pq_setkeepalivesinterval(newval, MyProcPort);
#else
...
#if 0
	snprintf(nbuf, sizeof(nbuf), "%d", gtm_pq_getkeepalivesinterval(MyProcPort));
#else
...
gtm_assign_tcp_keepalives_count(int newval, void *extra)
{
	/* See comments in assign_tcp_keepalives_idle */
#if 0
	(void) gtm_pq_setkeepalivescount(newval, MyProcPort);
#else
...
#if 0
	snprintf(nbuf, sizeof(nbuf), "%d", gtm_pq_getkeepalivescount(MyProcPort));
#else
...
config_enum_lookup_by_value(struct config_enum * record, int val)
{
	...
	if (isStartUp)
		gtm_write_stderr("could not find enum option %d for %s\n",
					 val, record->gen.name);
	else
