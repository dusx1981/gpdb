 ...
 * gtm_pq_getmessage, and then parsed and converted from that using the routines
 * in this module.
 *
 ...
 *		gtm_pq_beginmessage - initialize StringInfo buffer
 *		pq_sendbyte		- append a raw byte to a StringInfo buffer
 *		pq_sendint		- append a binary integer to a StringInfo buffer
 *		pq_sendint64	- append a binary 8-byte int to a StringInfo buffer
 *		gtm_pq_sendfloat4	- append a float4 to a StringInfo buffer
 *		gtm_pq_sendfloat8	- append a float8 to a StringInfo buffer
 *		gtm_pq_sendbytes	- append raw data to a StringInfo buffer
 *		gtm_pq_sendcountedtext - append a counted text string (with character set conversion)
 *		gtm_pq_sendtext		- append a text string (with conversion)
 *		gtm_pq_sendstring	- append a null-terminated text string (with conversion)
 *		gtm_pq_send_ascii_string - append a null-terminated text string (without conversion)
 *		gtm_pq_endmessage	- send the completed message to the frontend
 ...
 *		gtm_pq_puttextmessage - generate a character set-converted message in one step
 *		gtm_pq_putemptymessage - convenience routine for message with empty body
 *
 * Message parsing after input:
 *		gtm_pq_getmsgbyte	- get a raw byte from a message buffer
 *		gtm_pq_getmsgint	- get a binary integer from a message buffer
 *		gtm_pq_getmsgint64	- get a binary 8-byte int from a message buffer
 *		gtm_pq_getmsgfloat4 - get a float4 from a message buffer
 *		gtm_pq_getmsgfloat8 - get a float8 from a message buffer
 *		gtm_pq_getmsgbytes	- get raw data from a message buffer
 *		gtm_pq_copymsgbytes - copy raw data from a message buffer
 *		gtm_pq_getmsgtext	- get a counted text string (with conversion)
 *		gtm_pq_getmsgstring - get a null-terminated text string (with conversion)
 *		gtm_pq_getmsgend	- verify message fully consumed
...
/* --------------------------------
 *		gtm_pq_beginmessage		- initialize for sending a message
 * --------------------------------
 */
void
gtm_pq_beginmessage(StringInfo buf, char msgtype)
{
	initStringInfo(buf);
 ...
 */
void
gtm_pq_sendbytes(StringInfo buf, const char *data, int datalen)
{
 ...
 */
void
gtm_pq_sendcountedtext(StringInfo buf, const char *str, int slen,
 ...
 */
void
gtm_pq_sendtext(StringInfo buf, const char *str, int slen)
{
	appendBinaryStringInfo(buf, str, slen);
}
 ...
 */
void
gtm_pq_sendstring(StringInfo buf, const char *str)
{
	int			slen = strlen(str);
 ...
 */
void
gtm_pq_send_ascii_string(StringInfo buf, const char *str)
{
	while (*str)
	{
		char	ch = *str++;
 ...
 */
void
gtm_pq_sendfloat4(StringInfo buf, float4 f)
{
	union
	{
		float4		f;
 ...
 */
void
gtm_pq_sendfloat8(StringInfo buf, float8 f)
{
#ifdef INT64_IS_BUSTED
	union
	{
 ...
 *		gtm_pq_endmessage	- send the completed message to the frontend
 *
 * The data buffer is gtm_pfree()d, but if the StringInfo was allocated with
 * makeStringInfo then the caller must still gtm_pfree it.
 * --------------------------------
 */
void
gtm_pq_endmessage(Port *myport, StringInfo buf)
{
 ...
 */
void
gtm_pq_puttextmessage(Port *myport, char msgtype, const char *str)
{
	int			slen = strlen(str);
 ...
 */
void
gtm_pq_putemptymessage(Port *myport, char msgtype)
{
 ...
 */
int
gtm_pq_getmsgbyte(StringInfo msg)
{
	if (msg->cursor >= msg->len)
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("no data left in message")));
 ...
 */
unsigned int
gtm_pq_getmsgint(StringInfo msg, int b)
{
	unsigned int result;
	unsigned char n8;
	uint16		n16;
	...
	{
		case 1:
			gtm_pq_copymsgbytes(msg, (char *) &n8, 1);
			result = n8;
			break;
		case 2:
			gtm_pq_copymsgbytes(msg, (char *) &n16, 2);
			result = ntohs(n16);
			break;
		case 4:
			gtm_pq_copymsgbytes(msg, (char *) &n32, 4);
			result = ntohl(n32);
			break;
		default:
			elog(ERROR, "unsupported integer size %d", b);
 ...
 *		gtm_pq_getmsgint64	- get a binary 8-byte int from a message buffer
 *
 * It is tempting to merge this with gtm_pq_getmsgint, but we'd have to make the
 * result int64 for all data widths --- that could be a big performance
 * hit on machines where int64 isn't efficient.
 * --------------------------------
 */
int64
gtm_pq_getmsgint64(StringInfo msg)
{
	int64		result;
	uint32		h32;
	uint32		l32;

	gtm_pq_copymsgbytes(msg, (char *) &h32, 4);
	gtm_pq_copymsgbytes(msg, (char *) &l32, 4);
	h32 = ntohl(h32);
	l32 = ntohl(l32);
	...
	if ((result < 0) ? (h32 != -1) : (h32 != 0))
		ereport(ERROR,
				(ERANGE,
				 gtm_errmsg("binary value is out of range for type bigint")));
#else
	result = h32;
	result <<= 32;
	result |= l32;
 ...
 *		gtm_pq_getmsgfloat4 - get a float4 from a message buffer
 *
 * See notes for gtm_pq_sendfloat4.
 * --------------------------------
 */
float4
gtm_pq_getmsgfloat4(StringInfo msg)
{
	union
	{
		float4		f;
 ...
 *		gtm_pq_getmsgfloat8 - get a float8 from a message buffer
 *
 * See notes for gtm_pq_sendfloat8.
 * --------------------------------
 */
float8
gtm_pq_getmsgfloat8(StringInfo msg)
{
#ifdef INT64_IS_BUSTED
	union
	{
	...
	swap.h[0] = gtm_pq_getmsgint(msg, 4);
	swap.h[1] = gtm_pq_getmsgint(msg, 4);
#else
 ...
 */
const char *
gtm_pq_getmsgbytes(StringInfo msg, int datalen)
{
	const char *result;
	...
	if (datalen < 0 || datalen > (msg->len - msg->cursor))
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("insufficient data left in message")));
 ...
 */
void
gtm_pq_copymsgbytes(StringInfo msg, char *buf, int datalen)
{
	if (datalen < 0 || datalen > (msg->len - msg->cursor))
		ereport(ERROR,
				(EPROTO,
 ...
 */
char *
gtm_pq_getmsgtext(StringInfo msg, int rawbytes, int *nbytes)
{
	char	   *str;
	char	   *p;
 ...
 */
const char *
gtm_pq_getmsgstring(StringInfo msg)
{
	char	   *str;
	int			slen;
	...
	if (msg->cursor + slen >= msg->len)
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("invalid string in message")));
	msg->cursor += slen + 1;
 ...
 */
void
gtm_pq_getmsgend(StringInfo msg)
{
	if (msg->cursor != msg->len)
		ereport(ERROR,
				(EPROTO,
				 gtm_errmsg("invalid message format")));
}
