 ...
 *	as it is gtm_pfree()'d.  Otherwise we risk tying up a lot of memory in
 *	freelist entries that might never be usable.  This is specially needed
 *	when the caller is repeatedly gtm_repalloc()'ing a block bigger and bigger;
 ...
 * logically empty, because we don't attempt to detect gtm_pfree'ing the last
 * active chunk.
 */
 ...
 *		the units requested by palloc() and freed by gtm_pfree().  AllocChunks
 *		cannot be returned to malloc() individually, instead they are put
 *		on freelists by gtm_pfree() and re-used by the next palloc() that has
					  ...
					  bool isShared)
{
	...
	context = (AllocSet) Gtm_MemoryContextCreate(sizeof(AllocSetContext),
											 &AllocSetMethods,
											 parent,
											 name);
	...
	if (minContextSize > ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ)
	{
		...
		if (block == NULL)
		{
			Gtm_MemoryContextStats(TopMemoryContext);
			ereport(ERROR,
					(ENOMEM,
					 gtm_errmsg("out of memory"),
					 gtm_errdetail("Failed while creating memory context \"%s\".",
							   name)));
		}
		block->aset = context;
 ...
 *
 * This is called by Gtm_MemoryContextCreate() after setting up the
...
{
	/*
	 * Since Gtm_MemoryContextCreate already zeroed the context node, we don't
	...
	if (size > set->allocChunkLimit)
	{
		...
		if (block == NULL)
		{
			...
			ereport(ERROR,
					(ENOMEM,
					 gtm_errmsg("out of memory"),
					 gtm_errdetail("Failed on request of size %lu.",
							   (unsigned long) size)));
		}
		block->aset = set;
...
AllocSetRealloc(MemoryContext context, void *pointer, Size size)
{
	...
	else
	{
		 ...
		 * palloc/gtm_repalloc/gtm_pfree: the eventually freed chunks go into the
