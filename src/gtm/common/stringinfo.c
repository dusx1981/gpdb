 ...
 * to store data in it.
 *
 * NB: because we use gtm_repalloc() to enlarge the buffer, the string buffer
...
enlargeStringInfo(StringInfo str, int needed)
{
	...
	if (((Size) needed) >= (MaxAllocSize - (Size) str->len))
		ereport(ERROR,
				(ENOSPC,
				 ...
				 gtm_errdetail("Cannot enlarge string buffer containing %d bytes by %d more bytes.",
						   str->len, needed)));
	...
	str->data = (char *) gtm_repalloc(str->data, newlen);
